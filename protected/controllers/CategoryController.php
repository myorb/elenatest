...
class CategoryController extends Controller
{
	...
	public function actionUpdate($id)
	{
		...
		$post = Yii::app()->request->getPost('Category');

		if($post)
		{
			if ($post['price'] && $post['price'] != $model->price) {
				$descendants=$model->children()->findAll();
				foreach ($descendants as $value) {
					$chield = $this->loadModel($value->id);
					$chield->price = $post['price'];
					$chield->saveNode();
				}
			}
	...
	public function actionAdmin()
	{
		...
		//we must use order if we use tree greed view  
		$model->searchOrderCondition = true;
	...
	public function actionAjaxUpdateStatus($id)
	{
	    ...
	    if($model->saveNode()){
	    	echo $model->status==1?"active":"not active";
			Yii::app()->end();
		}
